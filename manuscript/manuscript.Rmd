...
refcats <- readxl::read_xlsx("../20230120 SRMA Datasheet Caspar.xlsx", sheet = 2, skip = 1)
refcats <- lapply(samples, function(s){
    refcats[, c(which(names(refcats) == s):(which(names(refcats) == s)+1))]
})
...
modres <- lapply(samples, function(p){
  ...
  refcat$Variable <- tolower(refcat$Variable)

  refcat <- refcat[refcat$Variable %in% names(dfmod[catvars]), ]
  
  dfmod[catvars] <- lapply(names(catvars)[catvars], function(c){
    v <- dfmod[[c]]
    relevel(v, ref = refcat$Reference[which(refcat$Variable == c)])
  })
